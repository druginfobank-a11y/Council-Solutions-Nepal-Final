  ...
  const getNavItems = () => {
    ...
    if (user.role === UserRole.STUDENT) {
      return [
        ...
        { label: 'Logic', path: '/program-hub', icon: lockedIcon(ICONS.Logic) }
      ];
    }
    if (user.role === UserRole.INSTRUCTOR) {
      return [...common, { label: 'Quiz Hub', path: '/quiz', icon: ICONS.Quiz }, { label: 'Intelligence', path: '/tutor', icon: lockedIcon(ICONS.Tutor) }];
    }
