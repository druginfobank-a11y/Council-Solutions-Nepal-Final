    ...
    <div className="max-w-md w-full space-y-10 animate-in relative z-10">
      ...
      <div className="bg-white/5 border border-white/5 p-8 rounded-[40px] backdrop-blur-xl text-center">
        <p className="text-slate-400 text-sm font-medium leading-relaxed mb-6">
          Your instructor credentials have been received. To maintain institutional standards, an administrator must verify your profile before you can architect academic resources.
        </p>
...
const ExpiredAccessScreen: React.FC<{ onLogout: () => void }> = ({ onLogout }) => (
  <div className="h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center relative overflow-hidden">
    ...
    <div className="max-w-md w-full space-y-10 animate-in relative z-10">
      ...
      <div className="bg-white/5 border border-white/5 p-8 rounded-[40px] backdrop-blur-xl">
        <p className="text-slate-400 text-sm font-medium leading-relaxed mb-8">
          Your initial 48-hour academic trial period has concluded. Advanced academic tools and AI tutoring now require an active subscription node.
        </p>
      ...
      <div className="max-w-md w-full space-y-8 animate-in relative z-10">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-[40px] shadow-sm">
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium leading-relaxed mb-8">
            Access to AI tutoring, vision analysis, and simulation theatre is restricted for new students. An administrator must verify your academic standing before advanced tools can be synchronized.
          </p>
          {requested ? (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-black/40 p-4 rounded-2xl text-left border border-white/5">
              ...
              <ol className="text-[10px] text-slate-500 space-y-1 list-decimal list-inside">
                ...
                <li>Add variable: <code className="text-white">GROQ_API_KEY</code></li>
                <li>Paste your Groq API key (Llama 3) and redeploy</li>
              </ol>
            </div>
          </div>
        ) : (
          <p className="text-slate-400 text-sm font-medium leading-relaxed mb-6 italic">"The intelligence satellite network requires a valid Groq API key to synchronize real-time academic data and AI tutoring."</p>
        )}
        <a href="https://console.groq.com/keys" target="_blank" rel="noopener noreferrer" className="text-[9px] font-black text-blue-400 uppercase tracking-widest hover:underline block mt-4">Get Groq API Key</a>
      </div>
      {!isVercel && (
              ...
              <Route path="/" element={<Layout user={user} onLogout={handleLogout} />}>
                {isExpired ? (
                  ...
                  <>
                    ...
                    {user.role === UserRole.INSTRUCTOR && <Route path="instructor" element={<InstructorDashboard user={user} />} />}
