...
// Groq Client Initialization (Llama 3 - Open Source Alternative)
const getGroqClient = () => {
  const key = String(
...
const handleAIError = (error: any) => {
  ...
  if (msg.includes("QUOTA") || msg.includes("429")) {
    throw new Error("Free limit exceed भयो। Gemini को free limit (15 requests per minute) सकियो। कृपया केही मिनेट पर्खनुहोस् वा Groq API Key (Llama 3) प्रयोग गर्नुहोस्।");
  }
...
export const generateMCQs = async (
  ...
  // Try Groq first if available and no file data (Groq is faster and has high free limits)
  if (groq && !fileData) {
    try {
      const prompt = `Generate ${count} high-yield MCQs for ${council} ${program}. Topic: ${topic}. Difficulty: ${difficulty}. Language: ${language === 'NEP' ? 'Nepali' : 'English'}. Return ONLY a JSON array of objects with question, options (array of 4), correctAnswer (index 0-3), and explanation.`;
      ...
      const mcqs = Array.isArray(result) ? result : (result.mcqs || result.questions || []);
      
      if (mcqs.length > 0) {
        await setCache("mcqs", cacheKey, mcqs);
        return mcqs;
      }
    } catch (e) {
      console.warn("Groq MCQ node fault, falling back to Gemini:", e);
    }
  }
...
export const getTutorResponse = async (
  ...
  if (groq) {
    try {
      const systemInstruction = `You are a specialized academic tutor for Nepalese Educational Councils (NPC, NMC, NNC, NHPC, NEC). Support English and Nepali.`;
      const completion = await groq.chat.completions.create({
        messages: [
      ...
      const result = { text: String(completion.choices[0].message.content || ""), node: 'llama-3-groq' };
      if (history.length === 0) await setCache("tutor", cacheKey, result);
      return result;
    } catch (e) {
      console.warn("Groq Tutor node fault, falling back to Gemini:", e);
    }
  }
  ...
  try {
    ...
    const result = { text: String(response.text || ""), node: isEng ? 'nec-engineering-flash' : 'gemini-1.5-flash' };
    if (history.length === 0) await setCache("tutor", cacheKey, result);
    return result;
  } catch (e) {
    throw handleAIError(e);
